...
export class AuthService {
  ...
  async activateProfile(profile: Profile): Promise<void> {
    ...
    if (!creds) {
      vscode.window.showErrorMessage(
        'Credentials file not found for this profile. Please sign in again.',
      );
      return;
    }
    ...
    vscode.window.showInformationMessage(
      `Switched to ${profile.email} (project: ${profile.project})`,
    );
  }
  ...
  async removeProfile(profile: Profile): Promise<void> {
    ...
    if (
      this.activeProfile?.email === profile.email &&
      this.activeProfile?.project === profile.project
    ) {
      this.signOut();
    }
  }
